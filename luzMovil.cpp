...
bool luzMovil::luz_ocupada[8] = {false};

luzMovil::luzMovil(unsigned int id, enum tipo_luz tipo, Tupla4f vec, Tupla4f ambiental, Tupla4f difuso, Tupla4f especular){
    assert(id >= 0 && id <= 7);
...
void luzMovil::desactivar(){
    glDisable(this->id_luz);
}

void luzMovil::set_direccion( float alpha, float beta ){
    this->alpha = alpha;
    this->beta  = beta;
}


void luzMovil::modificar_direccion( float alpha, float beta ){
    set_direccion(this->alpha + alpha, this->beta + beta);
}

void luzMovil::activar_direccion_polares(){
...
GLfloat luz_posicion[]={1.0, 1.0, 1.0 ,1.0};


luzMovil::luzMovil(void){
	angleconv=0;
	radio=8;
	angle=0;
	posx=radio*cos(angle);
	posz=radio*sin(angle);
	esfera=esfera.openply("perfilEsfera.ply");
	esfera.barridoST(esfera.getVertices(), 12);
	
}


void luzMovil::draw_luzM(){
	glMatrixMode(GL_MODELVIEW);
	glPushMatrix();
	glScaled(0.5,0.5,0.5);
	glTranslated(posx,5,posz);
	esfera.draw(SOLID,NEGRO);
...
void luzMovil::mover_dx(){
	angle=angle+0.2;
	posx=radio*cos(angle);
	posz=radio*sin(angle);
	draw_luzM();
}
void luzMovil::mover_sx(){
	angle=angle-0.2;
	posx=radio*cos(angle);
	posz=radio*sin(angle);
	draw_luzM();
